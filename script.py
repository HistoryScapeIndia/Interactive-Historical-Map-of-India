...
# Organize the historical data for the interactive map
historical_data = {
    "periods": [
        {
            "id": "ancient",
...
# Save the data as JSON for the web application
json_data = json.dumps(historical_data, indent=2)
print("Historical data organized successfully!")
print(f"Total events: {len(historical_data['events'])}")
print(f"Total monuments: {len(historical_data['monuments'])}")
print(f"Total periods: {len(historical_data['periods'])}")

# Save to file for reference
with open('historical_data.json', 'w') as f:
    f.write(json_data)

print("\nData saved to historical_data.json")
