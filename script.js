...
document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('historicalData.json')
        ...
        .then(data => {
            ...
            allEvents = [...data.events, ...data.monuments];
...
function loadFallbackData() {
    ...
    console.log('Using fallback data. Total events loaded:', allEvents.length);
...
function createMarker(event) {
    ...
    const marker = L.marker([event.latitude, event.longitude], { 
        icon: icon,
        eventData: event // Store event data directly on marker
    });
    ...
    marker.bindPopup(`
        <div class="popup-content">
            <h3>${event.name}</h3>
            <p><strong>Period:</strong> ${getPeriodName(event.period)}</p>
...
function getPeriodName(periodId) {
    if (!historicalData || !historicalData.periods) return periodId;
    
    const period = historicalData.periods.find(p => p.id === periodId);
...
function filterByPeriod(period) {
    ...
    // Add back only the markers that match the filter
...
function performSearch() {
    ...
    // Add back only the markers that match the search
    markers.forEach(marker => {
        const event = marker.options.eventData;
    ...
    // Zoom to show all matching markers if any were found
    if (foundMatches) {
    ...
    if (!foundMatches) {
        ...
        showNotification(`Found ${group.getLayers().length} event(s)`);
    }
}
